...
export interface WarrantiesTables {
  ...
  activityLog: any; jobAssignment: any; job: any; project: any; contact: any;
}
export interface WarrantiesServiceDeps { db: any; tables: WarrantiesTables }
export interface WarrantiesRoutesDeps {
  service: WarrantiesService
  authenticate: any
  requirePermission: (permission: string) => any
}

export function createWarrantiesService(deps: WarrantiesServiceDeps) {
  const { db, tables } = deps;
  const { warrantyTemplate, projectWarranty, warrantyClaim, activityLog, jobAssignment, job, project, contact } = tables;
 ...
 */
async function createWarrantyTemplate(companyId: string, data: {
  name: string;
  description?: string;
  category?: string;
 ...
 */
async function getWarrantyTemplates(companyId: string) {
 ...
 */
async function createProjectWarranty(companyId: string, data: {
  projectId: string;
  contactId?: string;
  templateId?: string;
  name: string;
 ...
 */
async function createWarrantiesFromTemplates(
  companyId: string,
  projectId: string,
  contactId: string,
  startDate: string,
 ...
 */
async function getProjectWarranties(projectId: string, companyId: string) {
 ...
 */
async function getActiveWarranties(
  companyId: string,
 ...
 */
async function getExpiringWarranties(companyId: string, { days = 30 }: { days?: number } = {}) {
 ...
 */
async function processExpiredWarranties(): Promise<number> {
 ...
 */
async function createClaim(companyId: string, data: {
  warrantyId: string;
  title: string;
  description?: string;
  location?: string;
 ...
 * Get single claim
 */
async function getClaim(claimId: string, companyId: string) {
 ...
 */
async function scheduleWarrantyWork(
  claimId: string,
  companyId: string,
 ...
 * Get warranty stats
 */
async function getWarrantyStats(companyId: string) {
  const now = new Date();
 ...
 */
async function getClaimsByCategory(
  companyId: string,
...
export type WarrantiesService = ReturnType<typeof createWarrantiesService>;

export function createWarrantiesRoutes(deps: WarrantiesRoutesDeps) {
  ...
  app.get('/templates', async (c) => {
    ...
    const templates = await service.getWarrantyTemplates(user.companyId);
  ...
  app.post('/templates', requirePermission('warranties:create'), async (c) => {
    ...
    const template = await service.createWarrantyTemplate(user.companyId, body);
    return c.json(template, 201);
  });

  app.post('/templates/seed', requirePermission('warranties:create'), async (c) => {
    const user = c.get('user') as any;
    await service.seedDefaultTemplates(user.companyId);
  ...
  app.get('/', async (c) => {
    ...
    const data = await service.getActiveWarranties(user.companyId, {
  ...
  app.get('/stats', async (c) => {
    const user = c.get('user') as any;
    const stats = await service.getWarrantyStats(user.companyId);
    return c.json(stats);
  });
  ...
  app.get('/expiring', async (c) => {
    ...
    const data = await service.getExpiringWarranties(user.companyId, {
  ...
  app.get('/project/:projectId', async (c) => {
    const user = c.get('user') as any;
    const data = await service.getProjectWarranties(c.req.param('projectId'), user.companyId);
    return c.json(data);
  });
  ...
  app.post('/', requirePermission('warranties:create'), async (c) => {
    const user = c.get('user') as any;
    const body = await c.req.json();
    const warranty = await service.createProjectWarranty(user.companyId, body);
  ...
  app.post('/from-templates', requirePermission('warranties:create'), async (c) => {
    ...
    const created = await service.createWarrantiesFromTemplates(
      user.companyId,
      projectId,
      contactId,
      startDate,
  ...
  app.get('/claims', async (c) => {
    ...
    const page = c.req.query('page');
    const limit = c.req.query('limit');
    const data = await service.getClaims(user.companyId, {
      warrantyId,
      projectId,
      status,
      priority,
  ...
  app.get('/claims/:id', async (c) => {
    const user = c.get('user') as any;
    const claim = await service.getClaim(c.req.param('id'), user.companyId);
  ...
  app.post('/claims/:id/deny', requirePermission('warranties:update'), async (c) => {
    const user = c.get('user') as any;
    const body = await c.req.json();
    const claim = await service.denyClaim(c.req.param('id'), user.companyId, {
      reason: body.reason,
      userId: user.userId,
    });
    return c.json(claim);
  ...
  app.get('/reports/by-category', async (c) => {
    ...
    const data = await service.getClaimsByCategory(user.companyId, {
      startDate,
      endDate,
    });
    return c.json(data);
