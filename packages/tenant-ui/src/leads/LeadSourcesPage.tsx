...
export function LeadSourcesPage({ api, toast, config }: { api: LeadsApi; toast?: LeadsToast; config?: LeadsConfig }) {
  ...
  const CopyField = ({ label, icon, value, field, mono = true, breakAll = false }: { label: string; icon: React.ReactNode; value: string; field: string; mono?: boolean; breakAll?: boolean }) => (
    <div style={{ marginBottom: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: c.muted, marginBottom: 6 }}>{icon} {label}</div>
  ...
  return (
    <div style={{ padding: '24px', maxWidth: 900, margin: '0 auto', color: c.text }} data-testid="lead-sources-shared">
      ...
      {showAdd && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }} onClick={() => setShowAdd(false)}>
          <div role="dialog" aria-label="Add Lead Source" style={{ background: c.surface, borderRadius: 12, padding: 24, width: '100%', maxWidth: 500, maxHeight: '80vh', overflow: 'auto' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {available.map((p) => (
                <button key={p.value} disabled={busy === `add:${p.value}`} onClick={() => addSource(p.value)}
                  style={{ padding: '14px 16px', border: `1px solid ${c.border}`, borderRadius: 8, background: c.surface, cursor: 'pointer', textAlign: 'left', display: 'flex', alignItems: 'center', gap: 12 }}
                  onMouseOver={(e) => (e.currentTarget.style.borderColor = p.color)} onMouseOut={(e) => (e.currentTarget.style.borderColor = c.border)}>
                  <div style={{ width: 8, height: 8, borderRadius: '50%', background: p.color }} />
                  <span style={{ fontWeight: 600, fontSize: 14 }}>{p.label}</span>
                </button>
              ))}
              {available.length === 0 && <p style={{ textAlign: 'center', color: c.faint, padding: 16, fontSize: 14 }}>All platforms connected!</p>}
            </div>
            <button onClick={() => setShowAdd(false)} style={{ marginTop: 16, padding: '8px 20px', border: `1px solid ${c.inputBorder}`, borderRadius: 6, background: c.surface, cursor: 'pointer', width: '100%', fontSize: 13 }}>Cancel</button>
          </div>
        </div>
      )}

      {loading && sources.length === 0 ? (
        <div style={{ padding: 40, textAlign: 'center', color: c.faint }}>Loading sources...</div>
      ) : sources.length === 0 ? (
        <div style={{ background: c.surface, borderRadius: 12, border: `1px solid ${c.border}`, padding: 40, textAlign: 'center' }}>
          <Settings size={48} style={{ marginBottom: 12, opacity: 0.3, color: c.faint }} />
          <div style={{ fontSize: 16, fontWeight: 600 }}>{error ? 'Lead sources could not be loaded' : 'No lead sources configured'}</div>
          {!error && <div style={{ fontSize: 13, color: c.muted, marginTop: 4 }}>Click "Add Source" to connect your first platform</div>}
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {sources.map((source) => {
            ...
            return (
              <div key={source.id} style={{ background: c.surface, borderRadius: 12, border: `1px solid ${c.border}`, overflow: 'hidden' }}>
