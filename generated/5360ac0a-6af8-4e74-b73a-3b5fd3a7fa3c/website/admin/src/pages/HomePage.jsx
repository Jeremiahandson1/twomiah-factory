...
// Scroll animations
  ...
  const phone = settings?.phone || '5555555555';
  const email = settings?.email || 'jeremiahandson@gmail.com';
  const address = settings?.address || '123 Main St';
  const city = settings?.city || 'EC';
  const state = settings?.state || 'WI';
  const zip = settings?.zip || '00000';
  
  // Homepage sections
  const hero = homepage?.hero || {};
  const trustBadges = homepage?.trustBadges?.filter(b => b.enabled) || [];
  const ctaSection = homepage?.ctaSection || {};
  const serviceAreas = homepage?.serviceAreas || ['EC', 'Nearby City 4', 'Nearby City 1', 'Nearby City 2', 'Nearby City 3'];
    ...
    <div className="home-page">
      ...
      >
        ...
        <div className={`hero-video-content ${hero.image ? 'hero-content' : ''}`}>
          <h1>{hero.title || 'EC Area Contractor'}</h1>
          ...
          <p className="hero-awards">{hero.description || 'Serving the EC with Quality Craftsmanship'}</p>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        "name": "Jeremiah",
        "image": "https://test@test.com/logo.jpg",
