  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-bold flex items-center gap-2"><AlertTriangle className="w-6 h-6 text-orange-500" />No-Show Tracking</h1><p className="text-sm text-gray-500 mt-1 dark:text-slate-400">Track missed visits to spot patterns and protect revenue</p></div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {Object.entries(REASON_COLORS).map(([reason, color]) => (
          <div key={reason} className="bg-white rounded-lg border p-4 dark:bg-slate-900">
            <div className="text-xs text-gray-500 uppercase dark:text-slate-400">{reason.replace(/_/g, ' ')}</div>
            ...
            {incidents.length === 0 ? <tr><td colSpan={5} className="px-4 py-12 text-center text-gray-400">No incidents logged.</td></tr> :
              incidents.map((i: any) => (
                <tr key={i.id} className="border-b">
                  ...
                  <td className="px-4 py-3 text-sm text-gray-600 max-w-xs truncate dark:text-slate-400">{i.notes || '—'}</td>
                </tr>
              ))}
          </tbody>
        </table>
