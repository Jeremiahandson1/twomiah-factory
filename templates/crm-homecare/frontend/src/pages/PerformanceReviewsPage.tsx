  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-bold flex items-center gap-2"><Award className="w-6 h-6 text-teal-600" />Performance Reviews</h1><p className="text-sm text-gray-500 mt-1 dark:text-slate-400">Quarterly and annual caregiver performance tracking</p></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {reviews.length === 0 ? <div className="col-span-full bg-white rounded-lg border p-12 text-center text-gray-400 dark:bg-slate-900">No performance reviews yet.</div> :
          reviews.map((r: any) => (
            <div key={r.id} className="bg-white rounded-lg border p-5 dark:bg-slate-900">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <div className="font-semibold">{r.caregiverName || r.caregiverId}</div>
                  <div className="text-xs text-gray-500 dark:text-slate-400">{r.reviewDate ? formatDate(r.reviewDate) : '—'} · by {r.reviewer || '—'}</div>
                </div>
