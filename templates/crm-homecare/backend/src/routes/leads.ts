...
app.post('/:id/convert', requirePermission('contacts:create'), async (c) => {
  ...
  await audit.log({ action: 'status_change', entity: 'lead', entityId: id, changes: { status: { old: existing.status, new: 'converted' } }, metadata: { contactId: newContact.id }, req: { user: currentUser } })
  emitToCompany(currentUser.companyId, EVENTS.REFRESH, { entity: 'lead' })
