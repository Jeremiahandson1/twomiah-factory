/**
 * SaaS Billing Service — Home Care (Care)
 *
 * Handles agency-level SaaS subscription billing (NOT per-client invoices —
 * that's handled by routes/billing.ts and routes/payments.ts).
 *
 * - Subscription management
 * - One-time purchases (self-hosted licenses)
 * - Usage tracking & overage billing
 * - Invoice generation (for the agency's own Twomiah Care subscription)
 * - Stripe integration
 *
 * Ported from crm/backend/src/services/billing.ts with one adaptation:
 * - Care's tenant table is `agencies` (not `company`). We alias the import
 *   to `company` so the rest of the service code reads unchanged.
 *
 * NOTE: subscription, billing_invoice, usage_record, addon_purchase tables
 * are created by migration 0009_add_saas_subscription_tables.sql and accessed
 * via raw SQL. Add them to db/schema.ts if you want Drizzle query-builder
 * support.
 */

import Stripe from 'stripe';
import { db } from '../../db/index.ts';
// In Care, the tenant-level table is `agencies`. Aliased to `company` so the
// ported code from crm/ reads without further changes.
