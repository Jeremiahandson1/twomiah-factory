 ...
 *
 * NOTE: trackingNumber and callLog tables use raw SQL for queries.
 */
 ...
 */
export async function createTrackingNumber(agencyId: string, data: {
  phoneNumber: string;
  forwardTo?: string;
  source?: string;
  campaign?: string;
  ...
  await db.execute(sql`
    INSERT INTO tracking_number (id, agency_id, phone_number, forward_to, source, campaign, medium, name, active, provider_id, provider, created_at, updated_at)
    VALUES (
      gen_random_uuid(), ${agencyId}, ${data.phoneNumber}, ${data.forwardTo || null},
 ...
 */
export async function updateTrackingNumber(numberId: string, agencyId: string, data: Record<string, unknown>) {
  ...
  for (const [key, value] of Object.entries(data)) {
    ...
    await db.execute(sql`UPDATE tracking_number SET ${sql.raw(`"${colName}"`)} = ${value}, updated_at = NOW() WHERE id = ${numberId} AND agency_id = ${agencyId}`);
  }
}
 ...
 */
export async function logCall(agencyId: string, data: {
  ...
  if (data.trackingNumber) {
    const tnRows = rows(await db.execute(sql`
      ...
      WHERE agency_id = ${agencyId} AND phone_number = ${data.trackingNumber}
      LIMIT 1
    `));
    if (tnRows[0]) {
  ...
  await db.execute(sql`
    INSERT INTO call_log (
      id, agency_id, tracking_number_id, client_id,
      ...
      gen_random_uuid(), ${agencyId}, ${trackingNumberId}, ${clientId},
  ...
  // If new caller, optionally create client
  if (isFirstTimeCaller && data.callerNumber && data.createContact !== false) {
    // Split callerName into firstName/lastName
    let firstName = 'Unknown';
    let lastName = 'Caller';
    if (data.callerName) {
      const parts = data.callerName.trim().split(/\s+/);
 ...
 * Get calls
 */
export async function getCalls(agencyId: string, {
  source,
  status,
  startDate,
  endDate,
 ...
 * Get single call
 */
export async function getCall(callId: string, agencyId: string) {
  const callRows = rows(await db.execute(sql`
    ...
    LEFT JOIN tracking_number tn ON cl.tracking_number_id = tn.id
    LEFT JOIN clients c ON cl.client_id = c.id
    WHERE cl.id = ${callId} AND cl.agency_id = ${agencyId}
    LIMIT 1
  `));
 ...
 */
export async function updateCall(callId: string, agencyId: string, data: Record<string, unknown>) {
  const allowedCols = ['notes', 'tags', 'is_lead', 'lead_value', 'client_id', 'status'];
  for (const [key, value] of Object.entries(data)) {
    const colName = key.replace(/[A-Z]/g, (m) => '_' + m.toLowerCase());
    if (!allowedCols.includes(colName)) continue;
    if (typeof value === 'object' && value !== null) {
      await db.execute(sql`UPDATE call_log SET ${sql.raw(`"${colName}"`)} = ${JSON.stringify(value)}::jsonb WHERE id = ${callId} AND agency_id = ${agencyId}`);
    } else {
      await db.execute(sql`UPDATE call_log SET ${sql.raw(`"${colName}"`)} = ${value} WHERE id = ${callId} AND agency_id = ${agencyId}`);
    }
  }
}
 ...
 */
export async function tagCallAsLead(callId: string, agencyId: string, { value, notes }: { value?: number; notes?: string }) {
  await db.execute(sql`
    UPDATE call_log SET
      is_lead = true,
      lead_value = ${value || null},
      notes = ${notes || null}
    WHERE id = ${callId} AND agency_id = ${agencyId}
  `);
}
 ...
 */
export async function handleCallRailWebhook(agencyId: string, payload: any) {
  const data = {
 ...
 */
export async function handleTwilioWebhook(agencyId: string, payload: any) {
  const data = {
