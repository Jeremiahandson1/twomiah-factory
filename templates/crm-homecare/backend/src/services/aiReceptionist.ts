...
import { aiReceptionistRule, aiReceptionistSettings, callLog, agencies, clients } from '../../db/schema.ts'
...
export async function getRules(agencyId: string) {
  return db.select().from(aiReceptionistRule)
    .where(eq(aiReceptionistRule.agencyId, agencyId))
    .orderBy(desc(aiReceptionistRule.createdAt))
}

export async function createRule(agencyId: string, data: {
  name: string
  trigger: string
  channel: string
...
export async function updateRule(ruleId: string, agencyId: string, data: Partial<{
  name: string
  trigger: string
  channel: string
  messageTemplate: string
  ...
  const [rule] = await db.update(aiReceptionistRule)
    ...
    .where(and(eq(aiReceptionistRule.id, ruleId), eq(aiReceptionistRule.agencyId, agencyId)))
    .returning()
  return rule
}

export async function deleteRule(ruleId: string, agencyId: string) {
...
export async function getSettings(agencyId: string) {
  const [settings] = await db.select().from(aiReceptionistSettings)
    .where(eq(aiReceptionistSettings.agencyId, agencyId))
    .limit(1)
...
export async function upsertSettings(agencyId: string, data: Partial<{
  isEnabled: boolean
 ...
 */
export async function evaluateRules(agencyId: string, callData: {
  callLogId: string
  callerNumber?: string
  callerName?: string
  callerEmail?: string
  ...
  const [agency] = await db.select({ name: agencies.name, settings: agencies.settings })
  ...
  const rules = await db.select().from(aiReceptionistRule)
    .where(and(eq(aiReceptionistRule.agencyId, agencyId), eq(aiReceptionistRule.isActive, true)))
  ...
  for (const rule of rules) {
      ...
      case 'new_lead':
        ...
        if (callData.callerNumber) {
          const [existing] = await db.select({ id: clients.id }).from(clients)
            .where(eq(clients.phone, callData.callerNumber))
            .limit(1)
    ...
    if (rule.delayMinutes > 0) {
      setTimeout(() => {
        sendAutoReply(rule.channel, callData.callerNumber, callData.callerEmail, message, agencyId)
      ...
      await sendAutoReply(rule.channel, callData.callerNumber, callData.callerEmail, message, agencyId)
...
export async function processRecordingWebhook(agencyId: string, payload: {
  ...
  const callRows = await db.execute(sql`
    SELECT id, caller_number, caller_name, client_id, status
    FROM call_log
    WHERE agency_id = ${agencyId} AND provider_id = ${payload.CallSid}
    LIMIT 1
  `)
  ...
  if (transcription) {
    ...
    const result = await summarizeAndGenerateReply(transcription, agency?.name || 'our agency', 'home care', callRow.caller_name)
    if (result?.summary) {
      await db.execute(sql`UPDATE call_log SET ai_summary = ${result.summary} WHERE id = ${callRow.id}`)
    }
  }

  // Step 3: Look up caller email if we have a client
  let callerEmail: string | undefined
  if (callRow.client_id) {
    const [c] = await db.select({ email: clients.email }).from(clients).where(eq(clients.id, callRow.client_id)).limit(1)
    callerEmail = c?.email || undefined
  }

  // Step 4: Evaluate rules
  await evaluateRules(agencyId, {
