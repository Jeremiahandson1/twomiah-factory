...
// ─── Public Inbound Endpoints (no auth — identified by email address or webhook secret) ───
...
app.post('/:id/convert', async (c) => {
  ...
  // Split name into first/last for crm-roof contact schema
  const nameParts = (existing.homeownerName || 'Unknown').trim().split(/\s+/)
  ...
  const [newContact] = await db.insert(contact).values({
    ...
    leadSource: existing.sourcePlatform,
    companyId: currentUser.companyId,
  }).returning()
