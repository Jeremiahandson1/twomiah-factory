      ...
      case 'new_lead':
        ...
        if (callData.callerNumber) {
          const [existing] = await db.select({ id: contact.id }).from(contact)
            .where(eq(contact.phone, callData.callerNumber))
            .limit(1)
...
export async function processRecordingWebhook(companyId: string, payload: {
  ...
  if (transcription) {
    ...
    const result = await summarizeAndGenerateReply(transcription, comp?.name || 'our company', 'roofing', callRow.caller_name)
    if (result?.summary) {
...
export async function transcribeExistingCall(companyId: string, callId: string) {
  ...
  const [comp] = await db.select({ name: company.name }).from(company).where(eq(company.id, companyId)).limit(1)
  const result = await summarizeAndGenerateReply(transcription, comp?.name || 'our company', 'roofing', callRow.caller_name || undefined)
  if (result?.summary) {
