# RV & Powersports Premium Website

Premium marketing site for RV, marine and powersports dealerships.

Cloned from website-premium-contractor; every shared mechanism (admin, bookings, blog,
analytics, security) is identical. Vertical differences live in data/ and
views/sections/.


The "premium tier" website template for construction / general contractor businesses.

## What makes this template different from `website-contractor`

`website-contractor` (the existing $19 template) uses a fixed EJS structure with token substitution — every site has the same hero shape, the same services layout, the same section order; only text and colors change.

`website-premium-contractor` renders pages from a JSON section-composition schema. `pages.sections` is an array of `{ type, variant, data }` entries that the home.ejs renderer iterates over and dispatches to per-section partials under `views/sections/<type>/<variant>.ejs`. The composer (AI or staff) decides which sections appear, in what order, and with what variant — different builds produce visibly different sites.
...
```bash
cd templates/website-premium-contractor
bun install
