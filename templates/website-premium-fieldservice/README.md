...
The "premium tier" website template for HVAC / plumbing / electrical / cleaning / appliance-repair service trades.

Visually distinct from `website-premium-contractor` (editorial green for design-build firms) — fieldservice goes punchier: deep-navy + service-orange palette, single geometric sans (Inter), tighter line-height, and sharper buttons. Reads as utility/action-oriented rather than refined/editorial. Same section-composition architecture and section partials, different theme + sample data.

## What makes this template different from `website-fieldservice`

`website-fieldservice` (the existing $19 template) uses a fixed EJS structure with token substitution — every site has the same hero shape, the same services layout, the same section order; only text and colors change.

`website-premium-fieldservice` renders pages from a JSON section-composition schema. `pages.sections` is an array of `{ type, variant, data }` entries that the home.ejs renderer iterates over and dispatches to per-section partials under `views/sections/<type>/<variant>.ejs`. The composer (AI or staff) decides which sections appear, in what order, and with what variant — different builds produce visibly different sites.
...
```
templates/website-premium-fieldservice/
