...
export default function PresentEstimateScreen() {
  const router = useRouter();
  const { id } = useLocalSearchParams<{ id: string }>();
  const { estimate, loadEstimate, setStatus, save } = useEstimate();
      ...
      case 'credibility':
        return (
          <PresentSlide title="Why Choose Us">
            <View className="flex-1 justify-center px-12">
              <Text className="text-2xl text-white mb-6">Licensed, bonded, and insured</Text>
              <Text className="text-2xl text-white mb-6">Manufacturer-certified installation</Text>
              <Text className="text-2xl text-white mb-6">Comprehensive warranty coverage</Text>
              <Text className="text-2xl text-white">Thousands of satisfied customers</Text>
            </View>
          </PresentSlide>
        );
        ...
        return (
          <PresentSlide title="1-Year Price Lock">
            <View className="flex-1 justify-center items-center px-12">
              <Text className="text-xl text-white/70 mb-4">Price valid for 1 year</Text>
              <Text className="text-6xl font-bold text-white mb-4">
                {formatCurrency(estimate.subtotal)}
              </Text>
              <Text className="text-lg text-white/50">Lock in today's costs</Text>
            </View>
          </PresentSlide>
        );
      ...
      case '30day':
        return (
          <PresentSlide title="30-Day Special">
            <View className="flex-1 justify-center items-center px-12">
              <Text className="text-xl text-white/70 mb-4">Sign within 30 days</Text>
              <Text className="text-6xl font-bold text-accent mb-4">
                {formatCurrency(estimate.subtotal * 0.95)}
              </Text>
              <Text className="text-lg text-white/50">5% discount</Text>
            </View>
          </PresentSlide>
        );
      ...
      case 'buy_today_explain':
        return (
          <PresentSlide title="Buy Today Advantage">
            <View className="flex-1 justify-center px-12">
              <Text className="text-2xl text-white mb-6">Commit today and save the most:</Text>
              <Text className="text-xl text-white/80 mb-3">- No return visit overhead</Text>
              <Text className="text-xl text-white/80 mb-3">- Priority scheduling</Text>
              <Text className="text-xl text-white/80 mb-3">- Maximum discount</Text>
              <Text className="text-xl text-white/80">- Guaranteed pricing</Text>
            </View>
          </PresentSlide>
        );
            ...
            <View className="flex-1 justify-center items-center px-12">
              ...
              {estimate.discount_amount > 0 && (
                <Text className="text-xl text-accent">
                  Save {formatCurrency(estimate.discount_amount)}
                </Text>
              )}
            </View>
          </PresentSlide>
      ...
      case 'financing':
        return (
          <PresentSlide title="Financing Options">
            <View className="flex-1 justify-center px-12">
              <Text className="text-2xl text-white mb-6">Monthly payments available</Text>
              <View className="bg-white/10 rounded-2xl p-6 mb-4">
                ...
                <Text className="text-4xl font-bold text-white mt-2">
                  {formatCurrency(estimate.total_price / 60)}/mo
                </Text>
              </View>
              <View className="bg-white/10 rounded-2xl p-6">
                <Text className="text-lg text-white/70">120 months at 6.99% APR</Text>
                <Text className="text-4xl font-bold text-white mt-2">
                  {formatCurrency((estimate.total_price * 1.0699) / 120)}/mo
                </Text>
              </View>
              <Text className="text-xs text-white/30 mt-4">*Subject to credit approval.</Text>
            </View>
          </PresentSlide>
        );
          ...
          <PresentSlide title="Ready to Get Started?">
            <View className="flex-1 justify-center items-center px-12">
              <Text className="text-2xl text-white mb-8 text-center">
                Lock in your price and get on the schedule.
              </Text>
              <TouchableOpacity
                className="bg-success px-12 py-5 rounded-2xl"
                onPress={() => {
                  save();
                  router.push(`/(app)/estimate/${estimate.id}/sign`);
                }}
                activeOpacity={0.8}
              >
                <Text className="text-white text-2xl font-bold">Sign Contract</Text>
              </TouchableOpacity>
  ...
  return (
    <View className="flex-1 bg-gray-900">
      ...
      <Pressable className="absolute left-0 top-0 bottom-0 w-16" onPress={goPrev} />
      <Pressable className="absolute right-0 top-0 bottom-0 w-16" onPress={goNext} />
