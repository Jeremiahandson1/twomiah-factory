...
The "premium tier" website template for roofing contractors and storm-restoration firms.

Visually distinct from the other premium templates (contractor's editorial green, fieldservice's navy + service-orange) — roofing goes midnight slate + signal red. The red is the "free inspection / call now" accent used sparingly so it stays loud. Headlines use condensed Oswald display sans (industrial/construction vernacular) instead of contractor's Fraunces. Buttons are squared with uppercase labels — durable feel rather than refined.
...
## What makes this template different from `website-roof`

`website-roof` (the existing $19 template) uses a fixed EJS structure with token substitution — every site has the same hero shape, the same services layout, the same section order; only text and colors change.

`website-premium-roofing` renders pages from a JSON section-composition schema. `pages.sections` is an array of `{ type, variant, data }` entries that the home.ejs renderer iterates over and dispatches to per-section partials under `views/sections/<type>/<variant>.ejs`. The composer (AI or staff) decides which sections appear, in what order, and with what variant — different builds produce visibly different sites.
...
```
templates/website-premium-roofing/
...
```bash
cd templates/website-premium-roofing
bun install
