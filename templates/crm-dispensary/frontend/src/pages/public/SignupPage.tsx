...
export default function SignupPage() {
  ...
  const handleStartTrial = async () => {
    // Trial is already started on signup (30-day trial built in).
            ...
            <tbody>
              {FEATURE_COMPARISON.map((group) => (
                <React.Fragment key={group.category}>
                  <tr>
                    <td colSpan={5} className="pt-4 pb-2 font-semibold text-gray-900 text-xs uppercase tracking-wider dark:text-slate-100">
                      {group.category}
                    </td>
                  </tr>
                        ...
                        return (
                          <td key={k} className={`text-center py-2 px-2 ${selectedPlan === k ? 'bg-orange-50/50' : ''}`}>
                            {val === true ? <Check className="w-4 h-4 text-green-500 mx-auto" /> :
                             ...
                             <span className="text-gray-700 font-medium dark:text-slate-200">{val}</span>}
                          </td>
                        );
                      })}
                    </tr>
