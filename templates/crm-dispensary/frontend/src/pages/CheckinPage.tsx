  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-slate-100">Customer Check-In & Queue</h1>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-lg shadow-sm p-4 flex items-center gap-3 dark:bg-slate-900">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-slate-400">Waiting</p>
          </div>
        </div>
        <div className="bg-white rounded-lg shadow-sm p-4 flex items-center gap-3 dark:bg-slate-900">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-slate-400">Avg Wait</p>
          </div>
        </div>
        <div className="bg-white rounded-lg shadow-sm p-4 flex items-center gap-3 dark:bg-slate-900">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-slate-400">Served Today</p>
          </div>
        </div>
      </div>
      ...
      {tab === 'queue' && (
        <div>
          {loading ? (
            ...
            <>
              {/* Active Queue */}
              <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3 dark:text-slate-400">Active Queue ({activeQueue.length})</h3>
              <div className="space-y-3 mb-8">
                {activeQueue.map((entry, index) => (
                  <div key={entry.id} className="bg-white rounded-lg shadow-sm p-5 border border-gray-100 dark:bg-slate-900">
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center text-lg font-bold text-gray-600 dark:bg-slate-800 dark:text-slate-400">
                          {index + 1}
                        </div>
                        <div>
                    ...
                    <table className="w-full">
                      <thead className="bg-gray-50 dark:bg-slate-900">
                        <tr>
                          ...
                          <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase dark:text-slate-400">Source</th>
                          <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase dark:text-slate-400">Wait Time</th>
                          <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase dark:text-slate-400">Completed At</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y">
                        {completedQueue.map(entry => (
                          <tr key={entry.id} className="hover:bg-gray-50">
                            ...
                            <td className="px-4 py-3 text-sm text-gray-600 dark:text-slate-400">{(entry.source || 'walk_in').replace('_', ' ')}</td>
                            <td className="px-4 py-3 text-sm text-gray-600 dark:text-slate-400">{formatWaitTime(entry.waitMinutes || 0)}</td>
      ...
      {tab === 'checkin' && (
        <div className="bg-white rounded-lg shadow-sm p-6 max-w-2xl space-y-6 dark:bg-slate-900">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-slate-100">New Check-In</h3>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1 dark:text-slate-200">Customer Name *</label>
            <input
              type="text"
      ...
      {tab === 'qr' && (
        <div className="bg-white rounded-lg shadow-sm p-6 max-w-lg mx-auto text-center space-y-6 dark:bg-slate-900">
          ...
          <div className="bg-gray-50 rounded-xl p-8 border-2 border-dashed border-gray-200 dark:bg-slate-900 dark:border-slate-700">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1 dark:text-slate-200">Self Check-In URL</label>
            <div className="flex gap-2">
              <input
                type="text"
                readOnly
                value={`${window.location.origin}/checkin/self`}
                className="flex-1 px-3 py-2 border border-gray-300 rounded-lg bg-gray-50 text-gray-600 text-sm dark:border-slate-700 dark:bg-slate-900 dark:text-slate-400"
              />
              <Button
                variant="secondary"
                onClick={() => {
