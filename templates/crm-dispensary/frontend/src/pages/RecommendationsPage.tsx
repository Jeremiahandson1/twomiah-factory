    ...
    <div>
            ...
            <>
              <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {trendingProducts.map(product => (
                  <div key={product.id} className="bg-white rounded-lg shadow-sm p-5 border border-gray-100 dark:bg-slate-900">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1">
                        <h3 className="font-semibold text-gray-900 line-clamp-1 dark:text-slate-100">{product.name}</h3>
                        <p className="text-sm text-gray-500 capitalize dark:text-slate-400">{product.category || '--'}</p>
                      </div>
      ...
      {tab === 'customer' && (
        <div>
          {/* Search */}
          <div className="mb-6">
            <label className="block text-sm font-medium text-gray-700 mb-1 dark:text-slate-200">Search for a customer</label>
            <div className="flex gap-2">
            ...
            {customerResults.length > 0 && (
              <div className="mt-2 bg-white border border-gray-200 rounded-lg shadow-lg divide-y max-h-48 overflow-y-auto dark:bg-slate-900 dark:border-slate-700">
      ...
      {tab === 'similar' && (
        <div>
          {/* Search */}
          <div className="mb-6">
            <label className="block text-sm font-medium text-gray-700 mb-1 dark:text-slate-200">Search for a product</label>
            <div className="flex gap-2">
              <div className="relative flex-1">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                <input
          ...
          {loading ? (
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {similarProducts.map((product, idx) => (
                <div key={product.id || idx} className="bg-white rounded-lg shadow-sm p-5 border border-gray-100 dark:bg-slate-900">
                  ...
                  <div className="flex flex-wrap gap-1 mb-2">
                    ...
                    {product.category && (
                      <span className="px-2 py-0.5 text-xs font-medium rounded-full bg-gray-100 text-gray-600 capitalize dark:bg-slate-800 dark:text-slate-400">
                        {product.category}
                      </span>
                    )}
                  </div>
            ...
            <>
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                <div className="bg-white rounded-lg shadow-sm p-5 border border-gray-100 dark:bg-slate-900">
                  <p className="text-sm text-gray-500 mb-1 dark:text-slate-400">Click Rate</p>
                  <p className="text-2xl font-bold text-gray-900 dark:text-slate-100">{performance.clickRate ?? 0}%</p>
                </div>
                <div className="bg-white rounded-lg shadow-sm p-5 border border-gray-100 dark:bg-slate-900">
                  <p className="text-sm text-gray-500 mb-1 dark:text-slate-400">Purchase Rate</p>
                  <p className="text-2xl font-bold text-gray-900 dark:text-slate-100">{performance.purchaseRate ?? 0}%</p>
                </div>
                <div className="bg-white rounded-lg shadow-sm p-5 border border-gray-100 dark:bg-slate-900">
                ...
                <div className="bg-white rounded-lg shadow-sm p-5 border border-gray-100 dark:bg-slate-900">
                  ...
                  <p className="text-2xl font-bold text-gray-900 dark:text-slate-100">{Number(performance.totalImpressions || 0).toLocaleString()}</p>
                </div>
              </div>
