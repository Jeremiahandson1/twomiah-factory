                  ...
                  {isManager && (
                    <div className="flex gap-2">
                      <button onClick={() => handleSwapAction(req.id, 'approve')}
                        className="flex items-center gap-1 px-3 py-1.5 bg-green-700 text-white rounded-lg text-sm hover:bg-green-800">
          ...
          <div>
            {editingShift && (
              <button onClick={() => { handleDeleteShift(editingShift.id); setShiftModal(false); }}
                className="px-4 py-2 text-red-700 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg text-sm">Delete Shift</button>
            )}
          </div>
