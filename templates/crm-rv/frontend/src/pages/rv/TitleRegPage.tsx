  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      <div className="flex items-center gap-3 mb-1">
        ...
        <div><h1 className="text-2xl font-bold">Title & Registration</h1><p className="text-sm text-gray-500">Generate the title/reg paperwork and submit to the DMV.</p></div>
      </div>

      <div className="bg-white rounded-xl border shadow-sm p-4 mt-4 space-y-3">
        <div><label className="text-xs font-medium text-gray-600">Deal / customer</label>
        ...
        {lead && <>
          <div className="text-sm text-gray-600">Buyer: <span className="font-medium text-gray-800">{lead.customerName}</span> · Unit: <span className="font-medium text-gray-800">{[lead.unitYear, lead.unitMake, lead.unitModel].filter(Boolean).join(' ') || '—'}</span></div>
      ...
      {result && (result.error
        ...
        : <div className="mt-4 bg-white rounded-xl border shadow-sm p-5">
          <div className="flex items-center gap-2 mb-3 flex-wrap"><CheckCircle2 className="text-green-600" /><span className="text-lg font-bold capitalize">{result.result?.status}</span><span className="text-sm text-gray-500">· {result.result?.refNumber} · {result.result?.state}</span>
          ...
          <div className="grid sm:grid-cols-2 gap-4 text-sm">
            <div><div className="font-semibold text-gray-700 mb-1">Fees</div>
              <div className="divide-y">
                <div className="flex justify-between py-1"><span className="text-gray-600">Title</span><span>{money(result.result?.fees?.title)}</span></div>
                <div className="flex justify-between py-1"><span className="text-gray-600">Registration</span><span>{money(result.result?.fees?.registration)}</span></div>
                <div className="flex justify-between py-1"><span className="text-gray-600">Plate</span><span>{money(result.result?.fees?.plate)}</span></div>
            ...
            <div><div className="font-semibold text-gray-700 mb-1">Required documents</div>
              <ul className="list-disc pl-5 text-gray-600 space-y-0.5">{(result.result?.checklist || []).map((c: string, i: number) => <li key={i}>{c}</li>)}</ul></div>
          </div>
        </div>)}
    </div>
  );
