  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      <div className="flex items-center gap-3 mb-1">
        ...
        <div className="flex-1"><h1 className="text-2xl font-bold">Rentals</h1><p className="text-sm text-gray-500">Reservations, contracts, and fleet utilization.</p></div>
      ...
      <div className="grid grid-cols-3 gap-3 mt-4">
        {cards.map(([l, v]: any) => (<div key={l} className="bg-white rounded-xl border shadow-sm p-4"><div className="text-xs text-gray-500">{l}</div><div className="text-2xl font-bold mt-1">{v}</div></div>))}
      </div>

      {show && <div className="mt-4 bg-white rounded-xl border shadow-sm p-4 grid sm:grid-cols-3 gap-3">
      ...
      <div className="mt-4 bg-white rounded-xl border shadow-sm overflow-hidden">
        <div className="px-4 py-2.5 border-b text-sm font-semibold text-gray-600">Reservations</div>
        <div className="overflow-x-auto"><table className="min-w-full text-sm">
          ...
          <tbody>
            ...
            {rentals.map((r, i) => (<tr key={i} className="border-t hover:bg-gray-50">
              <td className="px-4 py-2">{r.unit}</td><td className="px-4 py-2 text-gray-600">{r.customer}</td>
              <td className="px-4 py-2 text-gray-500 text-xs">{r.start}{r.end ? ` → ${r.end}` : ''} ({r.days}d)</td>
