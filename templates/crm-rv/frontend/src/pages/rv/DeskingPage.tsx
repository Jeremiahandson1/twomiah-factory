      ...
      <div className="grid md:grid-cols-2 gap-4 mt-4">
        <div className="bg-white text-gray-900 rounded-xl border shadow-sm p-4 space-y-2.5 dark:bg-slate-900 dark:text-slate-100">
          ...
          {inputs.map(([label, key]) => (
            <label key={key} className="flex items-center justify-between text-sm"><span className="text-gray-600 dark:text-slate-400">{label}</span>
        ...
        <div className="space-y-4">
          <div className="bg-white text-gray-900 rounded-xl border shadow-sm p-4 dark:bg-slate-900 dark:text-slate-100">
            ...
            <div className="text-sm divide-y">
              ...
              {netTrade !== 0 && <div className="flex justify-between py-1.5"><span className="text-gray-600 dark:text-slate-400">Net trade equity</span><span className={netTrade > 0 ? 'text-green-700' : ''}>{netTrade > 0 ? '-' : '+'}{money(Math.abs(netTrade))}</span></div>}
          ...
          <div className="bg-white text-gray-900 rounded-xl border shadow-sm overflow-hidden dark:bg-slate-900 dark:text-slate-100">
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>{[48, 60, 72].map((m) => (<tr key={m} className="border-t"><td className="px-4 py-2 text-gray-600 dark:text-slate-400">{m} mo</td>{[6.99, 9.99, 12.99].map((a) => <td key={a} className="px-4 py-2 text-right">{money(payment(financed, a, m))}</td>)}</tr>))}</tbody>
            </table>
          </div>
        </div>
      </div>
