    ...
    <div className="max-w-6xl mx-auto p-6">
      <div className="flex items-center gap-3 mb-1">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">Pick a fresh lead — AI drafts a personalized, inventory-aware email + text in seconds. Review, tweak, send.</p>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-[320px_1fr] gap-5 mt-5">
        ...
        <div className="bg-white rounded-xl border shadow-sm overflow-hidden">
          <div className="px-4 py-3 border-b text-sm font-semibold text-gray-600 flex items-center gap-2"><MessageSquare size={15} /> Leads ({leads.length})</div>
          <div className="max-h-[70vh] overflow-y-auto divide-y">
            ...
            {leads.map((l) => (
              <button key={l.id} onClick={() => draftFor(l)}
                ...
                <div className="text-xs text-gray-500 mt-0.5">{interestLabel(l)} {l.unitPrice ? `· ${money(l.unitPrice)}` : ''}</div>
        ...
        {/* Draft */}
        <div>
          {!selected && <div className="bg-white rounded-xl border p-10 text-center text-gray-400"><Sparkles className="mx-auto mb-2" /> Pick a lead to draft an instant response.</div>}

          {selected && (
            <div className="space-y-4">
              <div className="bg-white rounded-xl border shadow-sm p-4 flex items-center justify-between flex-wrap gap-2">
                <div>
                  <div className="font-semibold">{selected.customerName}</div>
                  <div className="text-xs text-gray-500">Interested in <span className="font-medium text-gray-700">{interestLabel(selected)}</span> · via {(selected.source || '').replace(/_/g, ' ')}</div>
                </div>
              ...
              {loading && !draft && <div className="bg-white rounded-xl border p-8 text-center text-gray-500"><Loader2 className="animate-spin mx-auto mb-2" /> AI is reading the lead + your inventory and writing the response…</div>}

              {draft && (
                <>
                  {/* Email */}
                  <div className="bg-white rounded-xl border shadow-sm overflow-hidden">
                    <div className="px-4 py-2.5 border-b bg-gray-50 flex items-center gap-2 text-sm font-semibold text-gray-700"><Mail size={15} /> Email</div>
                  ...
                  {/* SMS */}
                  <div className="bg-white rounded-xl border shadow-sm overflow-hidden">
                    <div className="px-4 py-2.5 border-b bg-gray-50 flex items-center gap-2 text-sm font-semibold text-gray-700"><MessageSquare size={15} /> Text message</div>
                    <div className="p-4 space-y-2">
