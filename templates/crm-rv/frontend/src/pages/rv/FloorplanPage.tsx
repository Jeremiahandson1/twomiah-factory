  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      <div className="flex items-center gap-3 mb-1">
        ...
        <div><h1 className="text-2xl font-bold">Floorplan</h1><p className="text-sm text-gray-500 dark:text-slate-400">New units financed on floorplan — curtailment, interest accrual, and payoff tracking.</p></div>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">
        {cards.map(([label, val, cls]: any) => (
          <div key={label} className="bg-white rounded-xl border shadow-sm p-4 dark:bg-slate-900">
      ...
      <div className="mt-4 bg-white rounded-xl border shadow-sm overflow-hidden dark:bg-slate-900">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {units.length === 0 && <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-500 dark:text-slate-400">{loading ? 'Loading…' : 'No floored units.'}</td></tr>}
              {units.map((u, i) => (
