...
import {
  Menu, X, Home, Users, Caravan, Target, Wrench, Bell, Settings,
  LogOut, ChevronDown, Sun, Moon, Compass,
  LifeBuoy, BookOpen, BarChart3, MessageSquare, Send
} from 'lucide-react';
...
// `feature` gates the item via hasFeature(); items without one are always shown.
const ALL_NAV_ITEMS = [
  { to: '/crm', icon: Home, label: 'Dashboard', exact: true },
  { to: '/crm/inventory', icon: Caravan, label: 'Inventory', feature: 'unit_inventory' },
  { to: '/crm/leads', icon: Target, label: 'Sales Pipeline', feature: 'deal_pipeline' },
  { to: '/crm/service', icon: Wrench, label: 'Service', feature: 'service_dept' },
  { to: '/crm/alerts', icon: Bell, label: 'Alerts', feature: 'service_dept' },
  { to: '/crm/reports', icon: BarChart3, label: 'Reports', feature: 'reports' },
  { to: '/crm/messages', icon: MessageSquare, label: 'Messages', feature: 'two_way_texting' },
  { to: '/crm/sequences', icon: Send, label: 'Sequences', feature: 'follow_up_sequences' },
...
export default function AppLayout() {
  ...
  // Hide nav items whose gating feature isn't enabled for this company.
      ...
      <aside className={`fixed inset-y-0 left-0 z-30 w-64 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 transform transition-transform duration-200 ${sidebarOpen || !isMobile ? 'translate-x-0' : '-translate-x-full'} ${!isMobile ? 'relative' : ''}`}>
        <div className="flex items-center justify-between h-16 px-4 border-b border-gray-200 dark:border-gray-800">
          <div className="flex items-center gap-2">
            ...
            <span className="text-lg font-bold text-gray-900 dark:text-white">Twomiah Roam</span>
          </div>
          {isMobile && (
