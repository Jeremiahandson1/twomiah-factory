...
app.get('/stats', requirePermission('invoices:read'), async (c) => {
  ...
  invoices.forEach(inv => {
    ...
    stats.paidAmount += Number(inv.amountPaid) - Number(inv.amountRefunded || 0)
    if (inv.status !== 'void' && inv.status !== 'refunded') stats.outstanding += balance
  })
  return c.json(stats)
})
...
app.post('/', requirePermission('invoices:create'), async (c) => {
  ...
  // Number from the highest existing invoice number, not the row count — a
  // count-based scheme restarted at INV-00008 while the books were in the 2000s
  // (M-03). Continue the existing sequence and keep a single, stable format.
