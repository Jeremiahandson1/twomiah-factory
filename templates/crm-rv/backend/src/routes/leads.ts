...
app.get('/', requirePermission('contacts:read'), async (c) => {
  ...
  if (search) {
    conditions.push(or(
      ilike(lead.customerName, `%${search}%`),
      ilike(lead.email, `%${search}%`),
      ilike(lead.unitInterest, `%${search}%`),
...
app.post('/:id/convert', requirePermission('contacts:create'), async (c) => {
  ...
  const [newContact] = await db.insert(contact).values({
    ...
    type: 'lead',
    notes: `Converted from ${existing.sourcePlatform} lead. Unit interest: ${existing.unitInterest || 'N/A'}. Budget: ${existing.budget || 'N/A'}. Description: ${existing.description || 'N/A'}`,
    companyId: currentUser.companyId,
  }).returning()
...
app.post('/inbound/email', async (c) => {
  ...
  const [newLead] = await db.insert(lead).values({
    ...
    sourceId: source.id,
    customerName: parsed.name || 'Unknown',
    email: parsed.email,
    phone: parsed.phone,
    unitInterest: parsed.unitInterest,
...
interface ParsedLead {
  ...
  email?: string
  phone?: string
  unitInterest?: string // the unit/model the lead asked about (maps to lead.unitInterest)
  location?: string
  budget?: string
  description?: string
}

// Recognized lead source platforms for RV / powersports dealers:
// rv_trader, rvusa, rvt, cycletrader, atvtrader, boattrader, adf_email, web, other
...
function parseLeadEmail(platform: string, subject: string, text: string, html: string): ParsedLead {
  ...
  switch (platform) {
    ...
    case 'rvusa':
    case 'rvt':
      return parseRvListingEmail(subject, content)
    case 'cycletrader':
    case 'atvtrader':
    case 'boattrader':
      return parsePowersportsListingEmail(subject, content)
    default:
      return parseGenericEmail(subject, content)
  }
}

// RV Trader / RVUSA / RVT listing-lead emails
function parseRvListingEmail(subject: string, content: string): ParsedLead {
  const nameMatch = content.match(/(?:Customer|Buyer|Name)[:\s]+([^\n]+)/i)
  const phoneMatch = content.match(/(?:Phone|Tel)[:\s]+([\d\-\(\)\s\+]+)/i)
  const emailMatch = content.match(/(?:Email)[:\s]+([^\s\n]+@[^\s\n]+)/i)
  const unitMatch = subject.match(/[-–]\s*(.+)$/) || content.match(/(?:Unit|RV|Listing|Model|Stock)[:\s]+([^\n]+)/i)
  const locationMatch = content.match(/(?:Location|Address|City)[:\s]+([^\n]+)/i)
  const descMatch = content.match(/(?:Message|Description|Details|Comments)[:\s]+([^\n]+(?:\n[^\n]+)*)/i)

  return {
    name: nameMatch?.[1]?.trim(),
    phone: phoneMatch?.[1]?.trim(),
    email: emailMatch?.[1]?.trim(),
    unitInterest: unitMatch?.[1]?.trim(),
    location: locationMatch?.[1]?.trim(),
    description: descMatch?.[1]?.trim(),
  }
}

// Cycle Trader / ATV Trader / Boat Trader powersports listing-lead emails
function parsePowersportsListingEmail(subject: string, content: string): ParsedLead {
  const nameMatch = content.match(/(?:Customer|Buyer|Name)[:\s]+([^\n]+)/i)
  const phoneMatch = content.match(/(?:Phone|Tel)[:\s]+([\d\-\(\)\s\+]+)/i)
  const emailMatch = content.match(/(?:Email)[:\s]+([^\s\n]+@[^\s\n]+)/i)
  const unitMatch = subject.match(/[-–]\s*(.+)$/) || content.match(/(?:Unit|Vehicle|Listing|Model|Stock)[:\s]+([^\n]+)/i)
...
function parseGenericEmail(subject: string, content: string): ParsedLead {
  const nameMatch = content.match(/(?:Name|Customer|Buyer|From)[:\s]+([^\n]+)/i)
  const phoneMatch = content.match(/(?:Phone|Tel|Mobile)[:\s]+([\d\-\(\)\s\+]+)/i)
  const emailMatch = content.match(/(?:Email)[:\s]+([^\s\n]+@[^\s\n]+)/i)
  const unitMatch = content.match(/(?:Unit|RV|Vehicle|Listing|Model|Stock)[:\s]+([^\n]+)/i)
  const descMatch = content.match(/(?:Message|Details|Description|Notes|Comments)[:\s]+([^\n]+(?:\n[^\n]+)*)/i)
...
function parseWebhookPayload(platform: string, body: any): ParsedLead {
  // Normalize webhook payloads from different RV / powersports lead sources
  switch (platform) {
    case 'rv_trader':
    case 'rvusa':
    case 'rvt':
      return {
        name: body.customer_name || body.buyer_name || body.name,
        email: body.customer_email || body.email,
        phone: body.customer_phone || body.phone,
        unitInterest: body.unit || body.listing_title || body.model || body.stock_number,
        location: body.zip_code || body.location || body.city,
        budget: body.budget,
        description: body.message || body.description || body.comments,
      }
    case 'cycletrader':
    case 'atvtrader':
    case 'boattrader':
      return {
        name: body.customer?.name || body.lead?.name || body.name,
        email: body.customer?.email || body.email,
        phone: body.customer?.phone || body.phone,
        unitInterest: body.listing?.title || body.unit || body.model,
        location: body.location || body.zip_code,
        budget: body.budget,
        description: body.message || body.comments || body.description,
      }
    default:
      return {
        name: body.name || body.customer_name || body.buyer_name,
        email: body.email || body.customer_email,
        phone: body.phone || body.customer_phone,
        unitInterest: body.unit || body.model || body.listing_title || body.stock_number,
        location: body.location || body.address || body.city || body.zip,
        budget: body.budget,
        description: body.description || body.message || body.comments || body.details,
      }
  }
}
