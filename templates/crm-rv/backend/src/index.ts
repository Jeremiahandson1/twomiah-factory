...
const salesLeadsRoutes = (await import('./routes/salesLeads.ts')).default
app.route('/api/sales-leads', salesLeadsRoutes)
const repairOrdersRoutes = (await import('./routes/repairOrders.ts')).default
...
const oemPartsRoutes = (await import('./routes/oemParts.ts')).default
...
const syndicationModule = await import('./routes/syndication.ts')
app.route('/api/syndication', syndicationModule.default)
// Public, unauthenticated marketplace feed (pulled by RV Trader / Cycle Trader / etc. by token)
app.route('/api/public/feed', syndicationModule.syndicationPublic)
...
const server = serve({ fetch: app.fetch, port: PORT }, (info) => {
  logger.info(`Twomiah Roam CRM running on port ${info.port}`, {
