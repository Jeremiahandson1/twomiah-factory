/**
 * Multi-Provider Email Service (Drizzle / TypeScript)
 *
 ...
 *
 * NOTE: This service does not use Prisma or Drizzle database queries --
 * it is purely an email-sending utility. Converted to TypeScript only.
 */
      ...
      <body><div class="container">
        ...
        <div class="content">
          ...
          ${data.upcomingJobs?.length ? `
            <h3>Upcoming Jobs</h3>
            <ul>${data.upcomingJobs.map((j: any) => `<li>${j.title} - ${j.date}</li>`).join('')}</ul>
          ` : ''}
...
/**
 * Send a raw (non-template) email — used by follow-up sequences where the body
 * is author-supplied rather than a registered template. Falls back to console
 * logging when no provider is configured (dev), mirroring `send`.
 */
export async function sendRaw(to: string, subject: string, body: string, options: Record<string, unknown> = {}) {
  const from = (options.from as any) || { name: FROM_NAME, address: FROM_EMAIL };
  if (!transporter) {
    console.log('\nEMAIL (dev mode):\nTo:', to, '\nSubject:', subject, '\n---\n', body, '\n---\n');
    return { success: true, dev: true };
  }
  const html = `<div style="font-family:-apple-system,Segoe UI,Roboto,sans-serif;font-size:15px;line-height:1.5;color:#222;white-space:pre-wrap">${body}</div>`;
  const result = await transporter.sendMail({ from, to, subject, html, text: body, ...options });
  logger.info('Email sent (raw)', { to, subject, provider: PROVIDER });
  return { success: true, messageId: result.messageId };
}
...
const emailService = {
  ...
  sendWelcome: (to: string, data: Record<string, unknown>) => send(to, 'welcome', data),
  sendTeamInvite: (to: string, data: Record<string, unknown>) => send(to, 'teamInvite', data),

  // Invoices
  sendInvoice: (to: string, data: Record<string, unknown>) => send(to, 'invoiceSent', data),
  sendInvoiceReminder: (to: string, data: Record<string, unknown>) => send(to, 'invoiceReminder', data),
  sendInvoiceOverdue: (to: string, data: Record<string, unknown>) => send(to, 'invoiceOverdue', data),
  sendPaymentReceived: (to: string, data: Record<string, unknown>) => send(to, 'paymentReceived', data),

  // Quotes
  sendQuote: (to: string, data: Record<string, unknown>) => send(to, 'quoteSent', data),
  sendQuoteApproved: (to: string, data: Record<string, unknown>) => send(to, 'quoteApproved', data),

  // Jobs
  sendJobAssigned: (to: string, data: Record<string, unknown>) => send(to, 'jobAssigned', data),
  sendJobStatusChanged: (to: string, data: Record<string, unknown>) => send(to, 'jobStatusChanged', data),
