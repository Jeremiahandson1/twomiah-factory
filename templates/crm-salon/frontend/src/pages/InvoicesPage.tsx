...
export default function InvoicesPage() {
  ...
  const columns = [
    ...
    { key: 'amountPaid', label: 'Balance', render: (v: unknown, r: Record<string, unknown>) => { if (r.status === 'void') return <span className="text-gray-400">Void</span>; if (r.status === 'refunded') return <span className="text-amber-700">Refunded</span>; const bal = Number(r.total) - Number(v || 0); return bal > 0 ? <span className="text-orange-600 font-medium">${bal.toLocaleString()}</span> : <span className="text-green-600">Paid</span>; } },
