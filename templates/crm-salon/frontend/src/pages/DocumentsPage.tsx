        ...
        return (
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-gray-100 rounded-lg flex items-center justify-center dark:bg-slate-800">
              {row.thumbnailUrl ? (
                <AuthImg src={row.thumbnailUrl as string} className="w-10 h-10 rounded-lg object-cover" />
              ) : (
      ...
      <div className="bg-white rounded-lg shadow-sm p-4 mb-6 dark:bg-slate-900">
        <div className="grid md:grid-cols-4 gap-4">
          ...
          >
            ...
            {documentTypes.map(t => (
              <option key={t} value={t}>{t.replace(/_/g, ' ').replace(/^\w/, (ch) => ch.toUpperCase())}</option>
            ))}
          </select>
          <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={() => setPreviewDoc(null)}>
          <div className="relative max-w-4xl max-h-[90vh] overflow-auto" onClick={(e: React.MouseEvent) => e.stopPropagation()}>
            ...
            {(previewDoc.mimeType as string)?.startsWith('image/') ? (
              <img src={previewUrl || undefined} alt={previewDoc.name as string} className="max-w-full max-h-[90vh] rounded-lg" />
            ) : (
