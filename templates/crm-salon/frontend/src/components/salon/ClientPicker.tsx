...
/**
 * Client (contact) picker — debounced search against /api/contacts?search=.
 * Used anywhere a client must be chosen: the book, the formula log, memberships.
 */
...
export function contactName(c: ContactLite): string {
...
export default function ClientPicker({ value, onChange, initialLabel }: ClientPickerProps) {
      ...
      {value && !open ? (
        <div className="flex items-center justify-between px-3 py-2 border rounded-lg bg-gray-50 dark:bg-slate-900">
          ...
          <button
            ...
            className="text-xs text-teal-700 hover:text-teal-800 dark:text-teal-300 dark:hover:text-teal-200"
          >
            Change
          </button>
        </div>
