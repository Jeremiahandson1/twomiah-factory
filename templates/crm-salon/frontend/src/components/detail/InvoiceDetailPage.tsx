...
export default function InvoiceDetailPage() {
  ...
  const [payment, setPayment] = useState<PaymentFormData>({ amount: '', method: 'card', reference: '', notes: '', tipAmount: '' });
  ...
  const handleRecordPayment = async () => {
    ...
    setRecording(true);
    try {
      await api.invoices.recordPayment(id, { ...payment, amount: Number(payment.amount), tipAmount: Number(payment.tipAmount) || 0 });
              ...
              <table className="w-full">
                <thead className="bg-gray-50 dark:bg-slate-800">
                  <tr>
                    ...
                    <th className="px-4 py-2 text-right text-xs">Tip</th>
                ...
                <tbody className="divide-y">
                  {invoice.payments!.map((p: PaymentRecord, i: number) => (
                    <tr key={i}>
                      ...
                      <td className="px-4 py-2 text-right text-gray-500">{Number(p.tipAmount) > 0 ? `${Number(p.tipAmount).toFixed(2)}` : '-'}</td>
      ...
      <Modal isOpen={paymentOpen} onClose={() => setPaymentOpen(false)} title="Record Payment" size="md">
        <div className="space-y-4">
          ...
          <div>
            ...
            <input type="number" step="0.01" min="0" value={payment.tipAmount} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setPayment({...payment, tipAmount: e.target.value})} className="w-full px-3 py-2 border rounded-lg dark:bg-slate-800 dark:border-slate-700" placeholder="0.00" />
            <p className="text-xs text-gray-500 mt-1">Gratuity for the stylist — recorded on the payment, not added to the invoice total.</p>
          </div>
          <div>
