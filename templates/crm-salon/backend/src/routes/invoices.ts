...
app.post('/:id/payments', requirePermission('invoices:update'), async (c) => {
  ...
  // tipAmount is gratuity on top of `amount`; it is recorded on the payment and never counts toward the invoice. (SALON: checkout with tips)
  const paymentSchema = z.object({ amount: z.number().positive(), method: z.enum(['card', 'cash', 'check', 'bank_transfer', 'stripe', 'other']), reference: z.string().optional(), notes: z.string().optional(), tipAmount: z.number().min(0).max(100000).optional() })
  ...
  const [newPayment] = await db.insert(payment).values({ ...data, amount: amount.toString(), tipAmount: round2(data.tipAmount || 0).toString(), invoiceId: id }).returning()
