...
app.post('/', requirePermission('invoices:create'), async (c) => {
  ...
  // An invoice with no client saved with Client "—", counted toward revenue, and accepted
  // payment. Require a client on create. (MONEY-04)
...
app.post('/:id/payments', requirePermission('invoices:update'), async (c) => {
  ...
  // tipAmount is gratuity on top of `amount`; it is recorded on the payment and never counts toward the invoice.
  const paymentSchema = z.object({ amount: z.number().positive(), method: z.enum(['card', 'cash', 'check', 'bank_transfer', 'stripe', 'other']), reference: z.string().optional(), notes: z.string().optional(), tipAmount: z.number().min(0).max(1000).optional() })
  ...
  const tipAmount = round2(data.tipAmount || 0)
  // A tip larger than five times the payment (and over $100) is almost always a typo. (SALON-N6)
  if (tipAmount > Math.max(amount * 5, 100)) return c.json({ error: `A $${tipAmount.toFixed(2)} tip on a $${amount.toFixed(2)} payment looks wrong — check the amount.` }, 400)
  ...
  await db.transaction(async (tx) => {
    ...
    const [newPayment] = await tx.insert(payment).values({ ...data, amount: amount.toString(), tipAmount: tipAmount.toString(), invoiceId: id } as any).returning()
