...
// off-site embed — so these four public routes get an explicit open policy.
...
try { app.route('/api/service-menu', (await import('./routes/serviceMenu.ts')).default) } catch (e) { console.error('[salon] service-menu route failed to mount', e) }
...
const server = serve({ fetch: app.fetch, port: PORT }, (info) => {
  logger.info(`Twomiah Salon CRM running on port ${info.port}`, {
