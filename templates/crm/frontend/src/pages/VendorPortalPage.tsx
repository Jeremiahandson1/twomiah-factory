...
export default function VendorPortalPage() {
  ...
  if (error && !data) return <div className="min-h-screen flex items-center justify-center bg-gray-50 text-red-600">{error}</div>;
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b px-6 py-4">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div>
            <div className="font-bold text-gray-900">{data.company?.name}</div>
            <div className="text-sm text-gray-500">Vendor portal — {data.vendor?.name}</div>
          </div>
          <div className="text-sm text-gray-500">{data.company?.phone}</div>
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto p-6 space-y-8">
        <section>
          ...
          <div className="space-y-2">
            {(data.purchaseOrders || []).map((po: Record<string, any>) => (
              <button key={po.id} onClick={() => viewPo(po.id)} className="w-full bg-white border rounded-lg px-4 py-3 flex items-center justify-between hover:border-gray-400 text-left">
                <div>
                  <span className="font-medium text-gray-900 mr-3">{po.number}</span>
                  {statusChip(po.status)}
                </div>
                <div className="text-sm text-gray-600">{money(po.total)}</div>
              </button>
            ))}
          </div>
        </section>
        ...
        <section>
          <h2 className="font-semibold text-gray-900 mb-3">Submit an invoice</h2>
          <div className="bg-white border rounded-lg p-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div className="sm:col-span-2 flex items-center gap-3">
              ...
              {invoiceMsg && <span className="text-sm text-gray-600">{invoiceMsg}</span>}
            </div>
          </div>
        </section>

        <section>
          <h2 className="font-semibold text-gray-900 mb-3">Your invoices</h2>
          {(data.bills || []).length === 0 && <p className="text-sm text-gray-400">No invoices on file.</p>}
          <div className="space-y-2">
            {(data.bills || []).map((b: Record<string, any>) => (
              <div key={b.id} className="bg-white border rounded-lg px-4 py-3 flex items-center justify-between">
                <div><span className="font-medium text-gray-900 mr-3">{b.number || '(no number)'}</span>{statusChip(b.status)}</div>
                <div className="text-sm text-gray-600">{money(b.amount)} <span className="text-gray-400">/ paid {money(b.amountPaid)}</span></div>
              </div>
            ))}
          </div>
        </section>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50" onClick={() => setOpenPo(null)}>
          <div className="bg-white rounded-xl max-w-lg w-full p-6 max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-1">
              <h3 className="font-bold text-gray-900">{openPo.number}</h3>
              {statusChip(openPo.status)}
            </div>
            {openPo.jobTitle && <p className="text-sm text-gray-500 mb-2">Job: {openPo.jobTitle}</p>}
            {openPo.shipTo && <p className="text-sm text-gray-500 mb-2">Ship to: {openPo.shipTo}</p>}
            <table className="w-full text-sm my-3">
              <thead><tr className="text-left text-gray-500"><th className="py-1">Item</th><th className="py-1 text-right">Qty</th><th className="py-1 text-right">Unit</th><th className="py-1 text-right">Total</th></tr></thead>
              <tbody>
            ...
            <div className="text-right text-sm text-gray-900 font-semibold mb-4">Total {money(openPo.total)}</div>
