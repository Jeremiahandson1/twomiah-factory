...
interface PreviewData {
  ...
  rowCount: number;
  columns: string[];
  // Backend returns the sample rows under `sample` (not `preview`).
...
export default function ImportPage() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const previewData = await api.upload(`/api/import/preview/${selectedType}`, formData) as PreviewData;
      if (previewData.valid === false) {
        setError(previewData.error || 'This file could not be read. Check that it is a valid CSV.');
        setPreview(null);
        return;
      }
            ...
            {selectedType === 'invoices' && (
              <label className="flex items-center gap-3 cursor-pointer">
                <input
                  ...
                  className="w-4 h-4 rounded border-gray-300 text-gray-900 dark:border-slate-700 dark:text-slate-100"
                />
                <span className="text-gray-700 dark:text-slate-200">Create customers that do not exist yet (recommended — an invoice with no customer is hard to use)</span>
              </label>
            )}
