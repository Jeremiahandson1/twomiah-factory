...
export default function EstimatorSettingsPage() {
  ...
  const [settings, setSettings] = useState({
    ...
    estimatorHeadline: 'Get Your Free Instant Estimate',
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSettings({
          ...
          estimatorHeadline: data.estimatorHeadline || 'Get Your Free Instant Estimate',
  ...
  const save = async () => {
    setSaving(true);
    try {
      const res = await fetch('/api/company/estimator', {
        method: 'PUT',
      ...
      <div className="max-w-3xl mx-auto px-6 py-6 space-y-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Embed a cost estimator on your website. Visitors enter their address and instantly see an estimated price range.
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border p-6">
          ...
          <p className="text-xs text-gray-500 mb-3">Set the per-square price range shown to visitors.</p>
