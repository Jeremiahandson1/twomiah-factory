  ...
  const jobsByDate = useMemo(() => {
    ...
    jobs.forEach((job: Record<string, unknown>) => {
      if (job.scheduledDate) {
        const dateKey = format(new Date(job.scheduledDate as string), 'yyyy-MM-dd');
              ...
              {calendarDays.map((day: Date) => {
                ...
                return (
                  ...
                  >
                    ...
                    {dayJobs.slice(0, 2).map((job: Record<string, unknown>, i: number) => (
                      <div key={i} className="text-xs truncate rounded px-1 py-0.5 mb-0.5" style={{ backgroundColor: `${primaryColor}30`, color: primaryColor }}>{job.title as string}</div>
                    ))}
          ...
          <CardBody className="p-0">
            {selectedJobs.length > 0 ? (
              <div className="divide-y divide-slate-800">
                {selectedJobs.map((job: Record<string, unknown>) => (
                  <div key={job.id as string} className="p-4 hover:bg-slate-800/30 transition-colors">
                    <div className="flex items-start gap-3">
                      <div className="w-1 h-12 rounded-full" style={{ backgroundColor: primaryColor }} />
                      <div className="flex-1">
                        <p className="font-medium text-white">{job.title as string}</p>
                        <div className="flex items-center gap-4 mt-2 text-xs text-slate-400">
                          {!!job.assignee && <div className="flex items-center gap-1"><User className="w-3 h-3" />{String(job.assignee)}</div>}
                          {!!job.estimatedHours && <div className="flex items-center gap-1"><Clock className="w-3 h-3" />{String(job.estimatedHours)}h</div>}
                        </div>
                        <div className="mt-2"><StatusBadge status={job.status as string} /></div>
                      </div>
                    </div>
                  </div>
                ))}
