      ...
      <Tabs value={activeTab} onChange={setActiveTab}>
                ...
                {calls.map((call) => (
                  <TableRow key={call.id}>
                    <TableCell><div className="flex items-center gap-2">
                      ...
                      <span className="text-slate-300">{call.direction || 'inbound'}</span>
                    </div></TableCell>
                    <TableCell className="font-medium text-white">{call.caller_number || call.callerNumber || '-'}</TableCell>
                    <TableCell className="text-slate-400">{call.duration ? `${Math.floor(call.duration / 60)}:${String(call.duration % 60).padStart(2, '0')}` : '-'}</TableCell>
                    <TableCell><StatusBadge status={call.status === 'completed' ? 'completed' : call.ai_response_sent ? 'pending' : 'in_progress'} /></TableCell>
                    <TableCell className="text-slate-400 text-sm max-w-xs truncate">{call.ai_summary || call.aiSummary || (call.transcription ? 'Transcribed' : '-')}</TableCell>
                    <TableCell className="text-slate-400 text-sm">{call.start_time || call.startTime ? format(new Date(call.start_time || call.startTime), 'MMM d, h:mm a') : '-'}</TableCell>
                  </TableRow>
                ))}
              </TableBody></Table>
            ) : (
        ...
        <TabsContent value="settings">
          <Card><CardHeader title="AI Receptionist Settings" /><CardBody className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div><label className="block text-sm text-slate-400 mb-1">Business Hours Start</label><Input type="time" value={settings.businessHoursStart || '09:00'} onChange={(e: any) => updateSettings({ businessHoursStart: e.target.value })} /></div>
              <div><label className="block text-sm text-slate-400 mb-1">Business Hours End</label><Input type="time" value={settings.businessHoursEnd || '17:00'} onChange={(e: any) => updateSettings({ businessHoursEnd: e.target.value })} /></div>
            </div>
            <div>
            ...
            <div><label className="block text-sm text-slate-400 mb-1">Default Greeting</label><Textarea value={settings.greetingText || ''} onChange={(e: any) => updateSettings({ greetingText: e.target.value })} placeholder="Hi, thanks for calling! We're currently away but will get back to you soon." rows={3} /></div>
            <div><label className="block text-sm text-slate-400 mb-1">Forwarding Number</label><Input value={settings.forwardingNumber || ''} onChange={(e: any) => updateSettings({ forwardingNumber: e.target.value })} placeholder="(555) 123-4567" /></div>
          </CardBody></Card>
        </TabsContent>
      </Tabs>
