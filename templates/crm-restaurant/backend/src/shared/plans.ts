// The events modules. These are what this CRM actually IS, so they belong in
// every tier - a venue on the cheapest plan still needs the pipeline and the BEO.
//
// startup/featureSync.ts only consults this file when a company boots with an
// empty enabledFeatures (normally the factory writes them at deploy). Without
// these ids in the fallback, that path would hand a venue the contractor
// starter set and hide the entire events sidebar.
const EVENTS_CORE = [
  'event_bookings', 'event_spaces', 'catering_menus', 'banquet_orders', 'event_deposits',
]
...
export const PLAN_FEATURES: Record<string, string[]> = {
  ...
  pro: [
    ...
    'route_optimization', 'review_requests', 'service_agreements',
  ...
  business: [
    ...
    'auto_clock', 'route_optimization', 'review_requests', 'service_agreements',
