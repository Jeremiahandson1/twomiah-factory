...
try { app.route('/api/event-spaces', (await import('./routes/eventSpaces.ts')).default) } catch (e) { console.error('[events] event-spaces route failed to mount', e) }
try { app.route('/api/menu-packages', (await import('./routes/menuPackages.ts')).default) } catch (e) { console.error('[events] menu-packages route failed to mount', e) }
...
const server = serve({ fetch: app.fetch, port: PORT }, (info) => {
  logger.info(`Twomiah Events CRM running on port ${info.port}`, {
