...
async function main() {
  ...
  if (existingArticles.length === 0) {
    ...
    const helpArticles = [
      { title: 'Getting Started', content: 'Welcome! Start by adding your event spaces and catering menus, then log enquiries and build them into confirmed events. Use the sidebar to move between modules.', category: 'Getting Started', isFaq: true, sortOrder: 1 },
      { title: 'Managing Events', content: 'Events run through a pipeline from Enquiry to Tentative to Confirmed to Completed. Open an event to set the space, guest count, menu and run of show.', category: 'Events', isFaq: true, sortOrder: 2 },
      { title: 'Event Spaces', content: 'Define your bookable spaces (with seated and standing capacities) on the Spaces page. Events are booked against a space so double-bookings are caught.', category: 'Events', isFaq: false, sortOrder: 3 },
      { title: 'Catering Menus', content: 'Build catering menus and per-head packages on the Menus page. Add menu lines to an event and they scale with the guest count.', category: 'Catering', isFaq: false, sortOrder: 4 },
      { title: 'Run of Show & BEO', content: 'Add timed steps to an event to build the Run of Show, then print the Banquet Event Order for the kitchen and floor team.', category: 'Events', isFaq: false, sortOrder: 5 },
      { title: 'Deposits & Payments', content: 'Schedule deposits and payments against an event and record them as they come in. Track the outstanding balance toward the food & beverage minimum.', category: 'Billing', isFaq: false, sortOrder: 6 },
    ]
  ...
  const DEFAULT_CATEGORIES = ['Appetizers', 'Entrees', 'Desserts', 'Beverages', 'Bar Packages', 'Room Hire & AV', 'Service Charges']
  const DEFAULT_STATUSES = ['Enquiry', 'Tentative', 'Confirmed', 'Completed', 'Cancelled']
