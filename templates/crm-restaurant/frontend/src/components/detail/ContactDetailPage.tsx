          ...
          {hasFeature('event_bookings') && (((contact as any).events?.length ?? 0) > 0) && (
            <div className="bg-white dark:bg-slate-900 rounded-lg shadow-sm">
              <div className="p-4 border-b flex items-center justify-between">
                <h2 className="font-semibold text-gray-900 dark:text-slate-100">Events</h2>
                <span className="text-sm text-gray-500 dark:text-slate-400">{(contact as any).events.length}</span>
              </div>
                ...
                {(contact as any).events.map((ev: any) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium text-gray-900 dark:text-slate-100">{ev.name || 'Untitled event'}</p>
                        <p className="text-sm text-gray-500 dark:text-slate-400">{ev.eventDate ? formatDate(ev.eventDate + 'T12:00:00') : ''}</p>
                      </div>
                    </div>
            ...
            <div className="space-y-4">
              ...
              {hasFeature('event_bookings') && (
                <div className="flex items-center justify-between">
                  <span className="text-gray-500 dark:text-slate-400">Events</span>
