        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3" aria-label="Sidebar">
          <ul className="space-y-1" role="list">
            {navItems.map((item, index) => {
              // Show section header if this item has a section and it's different from the previous
              ...
              return (
                <li key={item.to}>
                  ...
                  {item.external ? (
                    <a
                      href={`${(company?.website || '').replace(/\/+$/, '')}/admin`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors text-gray-700 hover:bg-gray-100 dark:text-slate-300 dark:hover:bg-slate-800"
                    >
