...
export default function InvoiceDetailPage() {
  ...
  const computedBalance = (invoice.status === 'void' || invoice.status === 'refunded') ? 0 : (Number(invoice.balance) || (Number(invoice.total || 0) - Number(invoice.amountPaid || 0)));
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className={`rounded-lg p-6 text-center ${computedBalance > 0 ? 'bg-red-50' : 'bg-green-50'}`}>
            ...
            <p className="text-gray-600 dark:text-slate-400">{computedBalance > 0 ? 'Balance Due' : invoice.status === 'refunded' ? 'Refunded' : Number(invoice.amountRefunded || 0) > 0 ? `Paid in Full · ${Number(invoice.amountRefunded).toFixed(2)} refunded` : 'Paid in Full'}</p>
          </div>
        </div>
      </div>
