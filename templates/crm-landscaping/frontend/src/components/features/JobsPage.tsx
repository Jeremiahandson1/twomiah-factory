...
function JobForm({ job, projects, team, onSave, onClose }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <Input label="Service Call Title" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} required />
      ...
      <div className="flex justify-end gap-3 pt-4"><Button type="button" variant="ghost" onClick={onClose}>Cancel</Button><Button type="submit">{job ? 'Update' : 'Create'} Service Call</Button></div>
    </form>
  );
}
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div><h1 className="text-2xl font-bold text-white">Service Calls</h1><p className="text-slate-400 mt-1">{jobs.length} total service calls</p></div>
        <Button onClick={() => setShowForm(true)} icon={Plus}>New Service Call</Button>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        {[
          { label: 'Total Service Calls', value: jobs.length, icon: Wrench },
      ...
      <Card>
        {filtered.length > 0 ? (
          <Table><TableHead><TableRow><TableHeader>Service Call</TableHeader><TableHeader>Project</TableHeader><TableHeader>Assignee</TableHeader><TableHeader>Scheduled</TableHeader><TableHeader>Status</TableHeader><TableHeader>Actions</TableHeader></TableRow></TableHead><TableBody>
            {filtered.map((j) => (
              <TableRow key={j.id}>
        ...
        ) : (<CardBody><EmptyState icon={Wrench} title="No service calls found" description="Create your first service call" action={<Button onClick={() => setShowForm(true)} icon={Plus}>New Service Call</Button>} /></CardBody>)}
      </Card>
      <Modal isOpen={showForm} onClose={() => { setShowForm(false); setEditJob(null); }} title={editJob ? 'Edit Service Call' : 'New Service Call'}><JobForm job={editJob} projects={projects} team={teamMembers} onSave={handleSave} onClose={() => { setShowForm(false); setEditJob(null); }} /></Modal>
      <ConfirmDialog isOpen={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={() => deleteJob(deleteTarget?.id)} title="Delete Service Call" message={`Delete "${deleteTarget?.title}"?`} confirmText="Delete" variant="danger" />
    </div>
  );
}
