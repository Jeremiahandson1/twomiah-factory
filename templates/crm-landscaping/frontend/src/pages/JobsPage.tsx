...
export default function JobsPage() {
  ...
  const load = useCallback(async () => {
    ...
    } catch (err) { toast.error('Failed to load service calls'); }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing) { await api.jobs.update(editing.id, payload); toast.success('Service call updated'); }
      else { await api.jobs.create(payload); toast.success('Service call created'); }
  ...
  const handleDelete = async () => {
    try { await api.jobs.delete(toDelete.id); toast.success('Service call deleted'); setDeleteOpen(false); load(); }
    catch (err) { toast.error(err.message); }
  };

  const handleStart = async (job) => {
    try { await api.jobs.start(job.id); toast.success('Service call started'); load(); }
    catch (err) { toast.error(err.message); }
  };
  ...
  const columns = [
    ...
    { key: 'scheduledDate', label: 'Scheduled', render: (v) => v ? new Date(String(v).split('T')[0] + 'T00:00:00').toLocaleDateString() : '-' },
  ...
  return (
    <div>
      <PageHeader title="Service Calls" action={<Button onClick={openCreate}><Plus className="w-4 h-4 mr-2 inline"/>New Service Call</Button>} />
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Service Call' : 'New Service Call'} size="lg">
      ...
      <ConfirmModal isOpen={deleteOpen} onClose={() => setDeleteOpen(false)} onConfirm={handleDelete} title="Delete Service Call" message={`Delete "${toDelete?.title}"?`} confirmText="Delete" />
    </div>
  );
}
