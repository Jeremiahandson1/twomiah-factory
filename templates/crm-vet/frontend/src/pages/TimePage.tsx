...
export default function TimePage() {
  ...
  const [form, setForm] = useState<TimeForm>({ date: new Date().toISOString().split('T')[0], hours: '', description: '', billable: true, projectId: '' });
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [resRaw, projResRaw] = await Promise.all([api.time.list({ page, limit: 25 }), api.projects.list({ limit: 100 })]);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      if (editing) { await api.time.update(editing.id as string, { ...form, hours: Number(form.hours) }); toast.success('Updated'); }
  ...
  const handleDelete = async () => { try { await api.time.delete((toDelete as Record<string, unknown>).id as string); toast.success('Deleted'); setDeleteOpen(false); load(); } catch (err) { toast.error((err as Error).message); } };
  ...
  const openEdit = (item: Record<string, unknown>) => { setEditing(item); setForm({ date: (item.date as string)?.split('T')[0] || '', hours: String(item.hours), description: (item.description as string) || '', billable: item.billable as boolean, projectId: (item.projectId as string) || '' }); setModalOpen(true); };

  const columns = [
    { key: 'date', label: 'Date', render: (v: unknown) => new Date(v as string).toLocaleDateString() },
    { key: 'user', label: 'User', render: (v: unknown) => v ? `${(v as Record<string, unknown>).firstName} ${(v as Record<string, unknown>).lastName}` : '-' },
    { key: 'project', label: 'Project', render: (v: unknown) => (v as Record<string, unknown>)?.name as string || '-' },
    { key: 'hours', label: 'Hours', render: (v: unknown) => Number(v).toFixed(1) },
    { key: 'description', label: 'Description' },
    { key: 'billable', label: 'Billable', render: (v: unknown) => v ? <span className="text-green-600">Yes</span> : <span className="text-gray-400">No</span> },
  ];
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Time Entry' : 'Log Time'} size="md">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><label className="block text-sm font-medium mb-1">Hours *</label><input type="number" step="0.5" value={form.hours} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setForm({...form, hours: e.target.value})} className="w-full px-3 py-2 border rounded-lg" /></div>
          </div>
          ...
          <label className="flex items-center gap-2"><input type="checkbox" checked={form.billable} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setForm({...form, billable: e.target.checked})} className="rounded" /><span className="text-sm">Billable</span></label>
        </div>
