...
function createRateLimiter(windowMs: number, max: number, countMethod?: (m: string) => boolean) {
  ...
  return async (c: Context, next: Next) => {
    // A bucket only counts the methods it owns, so read traffic can't drain the write budget.
...
// Separate, generous ceilings sized for real use: reads are cheap and constant (polling, page
// loads ~10 req each, several concurrent staff); writes are human-paced. Abuse is still capped,
// but browsing can no longer lock you out of saving.
...
try { app.route('/api/appointments', (await import('./routes/appointments.ts')).default) } catch (e) { console.error('[vet] appointments route failed to mount', e) }
try { app.route('/api/visits', (await import('./routes/visits.ts')).default) } catch (e) { console.error('[vet] visits route failed to mount', e) }
try { app.route('/api/vaccinations', (await import('./routes/vaccinations.ts')).default) } catch (e) { console.error('[vet] vaccinations route failed to mount', e) }
try { app.route('/api/prescriptions', (await import('./routes/prescriptions.ts')).default) } catch (e) { console.error('[vet] prescriptions route failed to mount', e) }
try { app.route('/api/lab-results', (await import('./routes/labResults.ts')).default) } catch (e) { console.error('[vet] lab-results route failed to mount', e) }
try { app.route('/api/wellness-plans', (await import('./routes/wellnessPlans.ts')).default) } catch (e) { console.error('[vet] wellness-plans route failed to mount', e) }
try { app.route('/api/reminders', (await import('./routes/reminders.ts')).default) } catch (e) { console.error('[vet] reminders route failed to mount', e) }
...
// for the canonical implementation + commentary.
...
const server = serve({ fetch: app.fetch, port: PORT }, (info) => {
  logger.info(`Twomiah Vet CRM running on port ${info.port}`, {
