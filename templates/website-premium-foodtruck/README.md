...
The "premium tier" website template for aesthetic-first businesses: food, hospitality, fitness, beauty, and events. The verticals where the website IS the pitch and photography carries most of the weight.

Cream + ink + terracotta palette with Playfair Display headlines for proper magazine-editorial feel. Generous whitespace. Light line-weights everywhere. Should feel like a printed cookbook or boutique-hotel brochure rather than a service-business website.
...
## What makes this template different from `website-showcase`

`website-showcase` (the existing $19 template) uses a fixed EJS structure with token substitution — every site has the same hero shape, the same services layout, the same section order; only text and colors change.

`website-premium-showcase` renders pages from a JSON section-composition schema. `pages.sections` is an array of `{ type, variant, data }` entries that the home.ejs renderer iterates over and dispatches to per-section partials under `views/sections/<type>/<variant>.ejs`. The composer (AI or staff) decides which sections appear, in what order, and with what variant — different builds produce visibly different sites.
...
```
templates/website-premium-showcase/
...
```bash
cd templates/website-premium-showcase
bun install
