...
export const backupsDir = process.env.NODE_ENV === 'production'
  ? path.join(__dirname, '../persistent/backups')
  : path.join(__dirname, '../backups')

const MAX_BACKUPS = 3
const MIN_HOURS_BETWEEN_BACKUPS = 23
const BACKUP_CHECK_INTERVAL = 6 * 60 * 60 * 1000
...
export function createBackup(force = false) {
  pruneOldBackups()
