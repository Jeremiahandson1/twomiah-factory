...
function AdminLayout({ children, title, subtitle, actions }: { children: React.ReactNode; title?: string; subtitle?: string; actions?: React.ReactNode }) {
    ...
    <div className={`admin-layout ${darkMode ? 'dark' : ''}`}>
      ...
      <aside className={`admin-sidebar ${sidebarOpen ? 'open' : ''}`}>
        ...
        <nav className="admin-nav">
          ...
          <Link to="/seo" className={`admin-nav-item ${isActive('/seo') ? 'active' : ''}`}>
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <circle cx="11" cy="11" r="8"></circle>
