  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        ...
        <Select label="Service Call" value={form.jobId} onChange={(e) => setForm({ ...form, jobId: e.target.value })} options={jobs.map(j => ({ value: j.id, label: j.title }))} placeholder="Select service call" />
      </div>
