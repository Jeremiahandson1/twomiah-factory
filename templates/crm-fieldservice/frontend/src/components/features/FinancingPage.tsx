...
function FinancingForm({ item, contacts, onSave, onClose }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <Input label="Service Description" value={form.jobDescription} onChange={(e) => setForm({ ...form, jobDescription: e.target.value })} placeholder="e.g. AC installation, furnace replacement, duct cleaning" />
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onChange={setActiveTab}>
        ...
        <TabsContent value="applications">
          ...
          <Card className="mt-4">
            {filtered.length > 0 ? (
              <Table><TableHead><TableRow><TableHeader>Customer</TableHeader><TableHeader>Amount</TableHeader><TableHeader>Term</TableHeader><TableHeader>Monthly</TableHeader><TableHeader>Service Call</TableHeader><TableHeader>Status</TableHeader><TableHeader>Actions</TableHeader></TableRow></TableHead><TableBody>
                {filtered.map((app) => (
                  <TableRow key={app.id}>
