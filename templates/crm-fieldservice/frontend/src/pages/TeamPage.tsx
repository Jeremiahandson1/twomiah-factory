      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Member' : 'Add Member'} size="md">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-sm font-medium mb-1">Job Title / Trade</label><input placeholder="e.g. Lead Technician (roster label — login access is set under Settings → Users)" value={form.role} onChange={(e) => setForm({...form, role: e.target.value})} className="w-full px-3 py-2 border rounded-lg" /></div>
