...
The "premium tier" website template for cannabis dispensaries.

The only dark-first template in the premium set. Charcoal-black background with sage-emerald + warm copper accents — closer to a high-end cocktail bar or specialty spirits brand than the "weed leaf green" stereotype. Cormorant Garamond display serif for refined editorial feel. The tone is curated/boutique retail, not lifestyle/relaxation cliché.
...
## What makes this template different from `website-dispensary`

`website-dispensary` (the existing $19 template) uses a fixed EJS structure with token substitution — every site has the same hero shape, the same services layout, the same section order; only text and colors change.

`website-premium-dispensary` renders pages from a JSON section-composition schema. `pages.sections` is an array of `{ type, variant, data }` entries that the home.ejs renderer iterates over and dispatches to per-section partials under `views/sections/<type>/<variant>.ejs`. The composer (AI or staff) decides which sections appear, in what order, and with what variant — different builds produce visibly different sites.
...
```
templates/website-premium-dispensary/
...
```bash
cd templates/website-premium-dispensary
bun install
