...
export default function FeaturesSettingsPage() {
  ...
  const { isAdmin, company, updateCompany } = useAuth();
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // changed features since login. (automotive's api.company has no get();
        // /api/auth/me returns { user, company }.)
        const fresh = (await api.getMe()) as { company?: { enabledFeatures?: string[] } } | null;
        const enabled = fresh?.company?.enabledFeatures ?? company?.enabledFeatures ?? [];
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      updateCompany({ enabledFeatures: ids }); // refresh in-memory company (same idiom as OnboardingWizard)
