...
The "premium tier" website template for in-home senior care and caregiving agencies.

Visually distinct from the trades-leaning premium templates: sage-green + dusty-rose palette (warm, gentle, calm — anti-industrial), Lora humanist serif on display (warm, considered, never clinical), pill-shaped buttons with no aggressive shadows, generous whitespace. The whole template reads "trust us with your mom."
...
## What makes this template different from `website-homecare`

`website-homecare` (the existing $19 template) uses a fixed EJS structure with token substitution — every site has the same hero shape, the same services layout, the same section order; only text and colors change.

`website-premium-homecare` renders pages from a JSON section-composition schema. `pages.sections` is an array of `{ type, variant, data }` entries that the home.ejs renderer iterates over and dispatches to per-section partials under `views/sections/<type>/<variant>.ejs`. The composer (AI or staff) decides which sections appear, in what order, and with what variant — different builds produce visibly different sites.
...
```
templates/website-premium-homecare/
...
```bash
cd templates/website-premium-homecare
bun install
