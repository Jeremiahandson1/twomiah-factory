/**
 * Provisions ONE throwaway Dispensary CRM (crm-dispensary, "Twomiah Leaf") test tenant,
 ...
 *     cd apps/api && bun run scripts/cleanup-dispensary-test.ts <tenantId>
 *
 * Run from apps/api so bun auto-loads .env:
 *     cd apps/api && bun run scripts/provision-dispensary-test.ts
 */
...
const KNOWN_PASSWORD = 'Leaf-test-pw-' + crypto.randomBytes(3).toString('hex') + '!'
const ADMIN_EMAIL = 'twomiah14@gmail.com'

async function main() {
  const supabase = createClient(process.env.SUPABASE_URL!, process.env.SUPABASE_SERVICE_ROLE_KEY!)
  const slug = 'leaftest-' + Date.now().toString(36) + '-' + crypto.randomBytes(2).toString('hex')
  const tenantId = crypto.randomUUID()
  const products = ['crm']
  const NAME = 'Lakeshore Wellness (test)'
  const fsFeatures = getFeaturesForTemplate('crm-dispensary').map(f => f.id)
  console.log('[provision] Dispensary features →', fsFeatures.join(', '))
  ...
  const config: any = {
    ...
    company: {
      ...
      domain: '', domainMode: 'skip', ownerName: 'Owner', industry: 'dispensary',
  ...
  await supabase.from('tenants').insert({
    ...
    industry: 'dispensary', city: 'Madison', state: 'WI', status: 'pending', products,
    plan: 'starter', is_test_tenant: true, domain: null,
  })

  console.log('[provision] Generating crm-dispensary zip…')
  ...
  const deploy = await deployCustomer(
    { id: tenantId, slug, name: NAME, industry: 'dispensary', products, config },
    zip.zipPath,
    { products },
  )
  ...
  console.log('')
  console.log('Teardown:    cd apps/api && bun run scripts/cleanup-dispensary-test.ts ' + tenantId)
