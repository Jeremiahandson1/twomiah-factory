/**
 * Provisions ONE throwaway Field-service CRM (crm-fieldservice, "Twomiah Wrench") test tenant,
 ...
 *     cd apps/api && bun run scripts/cleanup-fieldservice-test.ts <tenantId>
 *
 * Run from apps/api so bun auto-loads .env:
 *     cd apps/api && bun run scripts/provision-fieldservice-test.ts
 */
...
const KNOWN_PASSWORD = 'Wrench-test-pw-' + crypto.randomBytes(3).toString('hex') + '!'
const ADMIN_EMAIL = 'twomiah14@gmail.com'

async function main() {
  const supabase = createClient(process.env.SUPABASE_URL!, process.env.SUPABASE_SERVICE_ROLE_KEY!)
  const slug = 'wrenchtest-' + Date.now().toString(36) + '-' + crypto.randomBytes(2).toString('hex')
  const tenantId = crypto.randomUUID()
  const products = ['crm']
  const NAME = 'North Star Heating & Air (test)'
  const fsFeatures = getFeaturesForTemplate('crm-fieldservice').map(f => f.id)
  console.log('[provision] Fieldservice features →', fsFeatures.join(', '))
  ...
  const config: any = {
    ...
    company: {
      ...
      domain: '', domainMode: 'skip', ownerName: 'Owner', industry: 'hvac',
  ...
  await supabase.from('tenants').insert({
    ...
    industry: 'hvac', city: 'Madison', state: 'WI', status: 'pending', products,
    plan: 'starter', is_test_tenant: true, domain: null,
  })

  console.log('[provision] Generating crm-fieldservice zip…')
  ...
  const deploy = await deployCustomer(
    { id: tenantId, slug, name: NAME, industry: 'hvac', products, config },
    zip.zipPath,
    { products },
  )
  ...
  console.log('')
  console.log('Teardown:    cd apps/api && bun run scripts/cleanup-fieldservice-test.ts ' + tenantId)
