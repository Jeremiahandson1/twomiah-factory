 ...
 *     cd apps/api && bun run scripts/cleanup-salon-pair-test.ts <tenantId>
 *
 * Run from apps/api so bun auto-loads .env:
 *     cd apps/api && bun run scripts/provision-salon-pair-test.ts
 */
...
async function main() {
  ...
  const slug = 'salonpair-' + Date.now().toString(36) + '-' + crypto.randomBytes(2).toString('hex')
  const tenantId = crypto.randomUUID()
  // 'website' gates having a site at all; 'website-premium' selects the premium family within it.
const products = ['crm', 'website', 'website-premium']
  const NAME = 'Gilded Shears Salon (test)'
  ...
  console.log('Salon CRM+SITE pair deployed. Wait ~2-3 min for first boot.')
  console.log('━'.repeat(64))
  ...
  console.log('Teardown:    cd apps/api && bun run scripts/cleanup-salon-pair-test.ts ' + tenantId)
  const siteUrl = (deploy as any).services?.site?.url || (deploy as any).siteUrl || ('https://' + slug + '-site.onrender.com')
  console.log('SALONPAIR_RESULT ' + JSON.stringify({ tenantId, crmUrl, siteUrl, repoUrl: deploy.repoUrl, email: ADMIN_EMAIL, password: KNOWN_PASSWORD }))
}
