...
const KNOWN_PASSWORD = 'Roof-test-pw-' + crypto.randomBytes(3).toString('hex') + '!'
const ADMIN_EMAIL = 'twomiah14@gmail.com'

async function main() {
  const supabase = createClient(process.env.SUPABASE_URL!, process.env.SUPABASE_SERVICE_ROLE_KEY!)
  const slug = 'rooftest-' + Date.now().toString(36) + '-' + crypto.randomBytes(2).toString('hex')
  const tenantId = crypto.randomUUID()
  const products = ['crm']
  const NAME = 'Summit Ridge Roofing (test)'
  const roofFeatures = getFeaturesForTemplate('crm-roof').map(f => f.id)
  console.log('[provision] Roof features →', roofFeatures.join(', '))
  ...
  const config: any = {
    ...
    company: {
      ...
      domain: '', domainMode: 'skip', ownerName: 'Owner', industry: 'roofing',
    ...
    features: { crm: roofFeatures, website: [], paid_ads: false },
  ...
  console.log('[provision] Generating crm-roof zip…')
  ...
  const deploy = await deployCustomer(
    { id: tenantId, slug, name: NAME, industry: 'roofing', products, config },
    zip.zipPath,
    { products },
  )
  ...
  console.log('Roof CRM test tenant deployed. Wait ~2-3 min for first boot.')
  console.log('━'.repeat(64))
  ...
  console.log('ROOFTEST_RESULT ' + JSON.stringify({ tenantId, crmUrl, repoUrl: deploy.repoUrl, email: ADMIN_EMAIL, password: KNOWN_PASSWORD }))
}
