/**
 * Tears down a tenant provisioned by scripts/provision-rv-test.ts — deletes the
 ...
 *
 *     cd apps/api && bun run scripts/cleanup-rv-test.ts <tenantId>
 */
import { hardDeleteTestTenant } from '../src/services/testCleanup.ts'

const tenantId = process.argv[2]
if (!tenantId) {
  console.error('Usage: bun run scripts/cleanup-rv-test.ts <tenantId>')
  process.exit(1)
}
