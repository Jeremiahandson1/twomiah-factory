...
function generateDeployScript(workDir: string, config: GenerateConfig, products: string[]) {
  ...
  if (products.includes('crm')) script += 'cd crm/backend && bun install && bunx drizzle-kit migrate && bun db/seed.ts && cd ../..\ncd crm/frontend && bun install && bun run build && cd ../..\n'
  script += '\necho "✅ Done!"\n'
