...
export async function generate(config: GenerateConfig): Promise<GenerateResult> {
  ...
  try {
    ...
    if (products.includes('crm')) {
      ...
      const crmTemplate = crmIndustry === 'home_care' ? 'crm-homecare' : crmIndustry === 'field_service' ? 'crm-fieldservice' : 'crm'
...
function buildTokenMap(config: GenerateConfig, slug: string): Record<string, string> {
  ...
  return {
    ...
    '{{HERO_TAGLINE}}': config.content?.heroTagline || c.heroTagline || (industry === 'home_care' ? 'VA APPROVED PROVIDER' : industry === 'field_service' ? 'Your Trusted HVAC & Plumbing Professionals' : 'Trusted ' + industryLabel),
...
function generateReadme(workDir: string, config: GenerateConfig, tokens: Record<string, string>) {
  ...
  if (products.includes('crm')) {
    const crmDir = config.company?.industry === 'home_care' ? 'crm-homecare' : config.company?.industry === 'field_service' ? 'crm-fieldservice' : 'crm'
...
function generateDeployScript(workDir: string, config: GenerateConfig, products: string[]) {
  ...
  const crmScriptDir = config.company?.industry === 'home_care' ? 'crm-homecare' : config.company?.industry === 'field_service' ? 'crm-fieldservice' : 'crm'
