...
function copyAndInject(src: string, dest: string, tokens: Record<string, string>) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      if (TEXT_EXTS.has(ext) || base === '.env' || base === '.env.example' || base.endsWith('.template')) {
...
function generateDeployScript(workDir: string, config: GenerateConfig, products: string[]) {
  let script = '#!/bin/bash\nset -e\n\n'
  if (products.includes('website')) script += 'cd website && npm install && cd ..\n'
  if (products.includes('crm')) script += 'cd crm/backend && npm install && npx prisma migrate deploy && npx prisma db seed && cd ../..\ncd crm/frontend && npm install && npm run build && cd ../..\n'
  script += '\necho "✅ Done!"\n'
