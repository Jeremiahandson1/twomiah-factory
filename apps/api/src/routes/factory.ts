...
async function runDeploy(tenant: any, job: any, options: { region?: string; plan?: string; dbPlan?: string } = {}) {
  ...
  try {
    ...
    if (result.success || result.deployedUrl || result.apiUrl) {
      ...
      const emailTenant = { ...tenant, admin_password: (result as any).adminPassword || tenant.admin_password }
      notifyDeployComplete(emailTenant, { apiUrl: result.apiUrl, deployedUrl: result.deployedUrl, siteUrl: result.siteUrl, repoUrl: result.repoUrl, adsUrl: result.adsUrl }).catch(e => console.warn('[Email] Deploy complete notification failed:', e.message))
    } else {
...
function buildPremiumNav(
  ...
  // about.
  const order = ['emergency', 'menu', 'services', 'find-care', 'packages', 'quote', 'pricing', 'reservations', 'private-dining', 'strains', 'caregivers', 'coverage', 'projects', 'storm', 'schedule', 'catering', 'deals', 'visit', 'about']
